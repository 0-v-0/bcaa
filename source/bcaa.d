...
@nogc nothrow pure:
    hash_t getHash(in Key key) @safe => key.hashOf;

    bool equals(in Key k1, in Key k2) {
...
size_t nextpow2(size_t n) pure nothrow @nogc {
...
@nogc nothrow:
    ...
    void deallocate(void[] b) @system => free(b.ptr);
...
T[] makeArray(T, Allocator)(auto ref Allocator alloc, size_t length)
    => cast(T[])alloc.allocate(length * T.sizeof);

T* make(T, Allocator)(auto ref Allocator alloc)
    => cast(T*)alloc.allocate(T.sizeof).ptr;
...
@nogc nothrow:
    @property pure @safe {
        ...
        size_t dim() const => buckets.length;

        size_t mask() const => dim - 1;
    }
    ...
    void set(in K key, in V val) {
        ...
        if (p.deleted) {
            p.entry.key = cast(K)key;
            p.entry.val = cast(V)val;
        } else {
            Node* newNode = allocator.make!Node();
            newNode.key = cast(K)key;
    ...
    private static size_t calcHash(in K pkey) pure {
    ...
    static if (isSomeString!K)
        @property {
            auto opDispatch(K key)() => opIndex(key);

            auto opDispatch(K key)(in V value) => opIndexAssign(value, key);
        }
    ...
    nothrow @nogc:
        ...
        bool empty() const pure @safe => len == 0;
    ...
    auto byKeyValue() => BCAARange!"entry"(buckets, length, firstUsed);

    auto byKey() => BCAARange!"entry.key"(buckets, length, firstUsed);

    auto byValue() => BCAARange!"entry.val"(buckets, length, firstUsed);
}
...
unittest {
...

    {
        ...
        assert(aa1.remove("Ferhat"));
        assert(aa1.Ferhat == null);
        assert(!aa1.remove("Foe"));
